...
class libro_ventas(models.TransientModel):
    ...
    def generate_xls_report(self):
        ...
        self.invoices = self.env['account.move'].search(
            [('invoice_date', '>=', self.date_from), ('invoice_date', '<=', self.date_to), ('type', '!=', 'in_invoice'),
             ('type', '!=', 'in_refund'), ('state', '!=', 'draft'), ('state', '!=', 'cancel')],
        ...
        for invoice in lines:
            ...
            for p in range(cont_impri):
                ...
                # Fecha del documento
                fech_doc = datetime.strftime(datetime.strptime(invoice.invoice_date, DEFAULT_SERVER_DATE_FORMAT),
                                             "%d/%m/%Y")
        ...
        row += 1
        ws1.write_merge(row, row, 11, 15, "Ventas Internas Afectadas solo Alicuota General + Adicional",
                        sub_header_style)
        ...
        ws1.write(row, col + 16, invoice.float_format(
            (base_general + base_genel_mas_adicional + base_reducido + tot_base_exentas + tot_base_no_gra)),
                  result_right)
        ws1.write(row, col + 17, invoice.float_format(
            (tax_general + tax_genel_mas_adicional + tax_reducido + tot_iva_exentas + tot_iva_no_gra)), result_right)
        ws1.write(row, col + 18, invoice.float_format(
            (ret_general + ret_genel_mas_adicional + ret_reducido + tot_reten_exent + tot_reten_no_gra)), result_right)
