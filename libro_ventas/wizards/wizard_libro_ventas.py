...
class LibroVentasModelo(models.Model):
    ...
    vat_ret_id = fields.Many2one('retention.iva', string='Nro de Comprobante IVA')
...
class libro_ventas(models.TransientModel):
    ...
    facturas_ids = fields.Many2many('account.move', string='Facturas',
                                    store=True)  ##Relacion con el modelo de la vista de la creacion de facturas
    retiva_ids = 0  ## Malo
    ...
    state = fields.Selection([('choose', 'choose'), ('get', 'get')],
                             default='choose')  ##Genera los botones de exportar xls y pdf como tambien el de cancelar
    ...
    def print_facturas(self):
        ...
        self.get_invoice2(action)
        self.line = self.env['account.wizard.pdf.ventas'].search([])
        return {'type': 'ir.actions.report', 'report_name': 'libro_ventas.reporte_factura_clientes',
                'report_type': "qweb-pdf"}

    def actualiza_fecha_voucher(self):
        lista = self.env['retention.iva'].search([
            ('iva_date', '>=', self.date_from),
            ('iva_date', '<=', self.date_to),
            ('state', '=', 'done'),
            ('move_type', 'in', ('out_invoice', 'out_refund', 'out_receipt'))
        ])
        for dett in lista:
            state = dett.state
            voucher_delivery_date = dett.iva_date
            # raise UserError(_('state: %s')%dett.invoice_id.id)
            lista_resumen = self.env['alicuota'].search([('move_id', '=', dett.move_id.id)])
            # raise UserError(_('cedula: %s')%lista_resumen)
            for deta in lista_resumen:
                self.env['alicuota'].browse(deta.id).write({
                    # 'fecha_comprobante':voucher_delivery_date,
                    # 'state_voucher_iva':state,
                    'date_voucher': voucher_delivery_date,
                    'retention_iva_state': state,
                })
    ...
    def generate_xls_report(self):
        ...
        ws1.write_merge(row, row, 2, 4,
                        str(self.company_id.partner_id.doc_type.upper()) + '-' + str(self.company_id.partner_id.vat),
                        sub_header_content_style)
        row += 1
        ...
        for invoice in self.line.sorted(key=lambda x: (x.name, x.invoice_ctrl_number), reverse=False):
            ...
            if invoice.state_retantion == 'done':
            ...
            if invoice.partner.taxpayer == 'True':
                if invoice.invoice_id.partner_id.vendor != 'international':
            ...
            # ********** NO CONTRIBUYENTES ********************
            if invoice.partner.taxpayer == 'False':
                if invoice.invoice_id.partner_id.vendor != 'international':
            ...
            if invoice.vat_ret_id.state == 'done':
                if invoice.invoice_id.partner_id.vendor != 'international':
        ...
        row = row + 1
        ws1.write_merge(row, row, 0, 2, "RESUMEN DE VENTAS", sub_header_style_c)
        ws1.write_merge(row, row, 3, 4, "Base Imponible", sub_header_style_c)
        ws1.write_merge(row, row, 5, 6, "Débito Fiscal", sub_header_style_c)
        ws1.write_merge(row, row, 7, 9, "Iva Retenidos por Ventas", sub_header_style_c)

        # ************* fila exentas o exoneradas *********
        row = row + 1
        ws1.write_merge(row, row, 0, 2, "Ventas internas Exentas o Exoneradas", right)
        ws1.write_merge(row, row, 3, 4, acum_exento, right)
        total_bases = total_bases + acum_exento
        ws1.write_merge(row, row, 5, 6, "0.00", right)
        ws1.write_merge(row, row, 7, 9, "0.00", right)

        # ************* fila SOLO ALICUOTA GENERAL *********
        row = row + 1
        # acum_ret_general=69
        ws1.write_merge(row, row, 0, 2, "Ventas Internas Afectadas sólo Alícuota General", right)
        ws1.write_merge(row, row, 3, 4, acum_base_general, right)
        total_bases = total_bases + acum_base_general
        ws1.write_merge(row, row, 5, 6, acum_general, right)
        total_debitos = total_debitos + (acum_general)
        ws1.write_merge(row, row, 7, 9, acum_ret_general, right)
        total_retenidos = total_retenidos + acum_ret_general

        # ************* fila ALICUOTA GENERAL MAS ADICIONAL *********
        row = row + 1
        ws1.write_merge(row, row, 0, 2, "Ventas Internas Afectadas sólo AlícuotaGeneral + Adicional", right)
        ws1.write_merge(row, row, 3, 4, acum_base_adicional, right)
        total_bases = total_bases + acum_base_adicional
        ws1.write_merge(row, row, 5, 6, acum_adicional, right)
        total_debitos = total_debitos + acum_adicional
        ws1.write_merge(row, row, 7, 9, acum_ret_adicional, right)
        total_retenidos = total_retenidos + acum_ret_adicional

        # ************* fila REDUCIDA *********
        row = row + 1
        ws1.write_merge(row, row, 0, 2, "Ventas Internas Afectadas sólo Alícuota Reducida", right)
        ws1.write_merge(row, row, 3, 4, acum_base_reducida, right)
        total_bases = total_bases + acum_base_reducida
        ws1.write_merge(row, row, 5, 6, acum_reducida + acum_reducida2, right)
        total_debitos = total_debitos + (acum_reducida + acum_reducida2)
        ws1.write_merge(row, row, 7, 9, acum_ret_reducida, right)
        total_retenidos = total_retenidos + acum_ret_reducida

        # ************* fila EXPORTACION *********
        row = row + 1
        ws1.write_merge(row, row, 0, 2, "Ventas de Exportación", right)
        ws1.write_merge(row, row, 3, 4, acum_fob, right)
        ...
        # ************* fila totales*********
        row = row + 1
        ws1.write_merge(row, row, 0, 2, "TOTAL:", right)
        ws1.write_merge(row, row, 3, 4, total_bases, right)
        ws1.write_merge(row, row, 5, 6, total_debitos, right)
        ws1.write_merge(row, row, 7, 9, total_retenidos, right)
