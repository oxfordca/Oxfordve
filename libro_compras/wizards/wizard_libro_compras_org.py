...
class libro_compras(models.TransientModel):
    ...
    def print_facturas(self):
        self.get_invoice()
        return {'type': 'ir.actions.report', 'report_name': 'libro_compras.libro_factura_proveedores',
    ...
    def generate_xls_report(self):
        ...
        invoices = self.env['account.move'].search(
            [('date', '>=', self.date_from), ('date', '<=', self.date_to), ('type', '!=', 'out_invoice'),
             ('type', '!=', 'out_refund'), ('state', '!=', 'draft'), ('state', '!=', 'cancel')],
            order='date ASC')  # order sirve para ordenar de manera ascendente o descendete los valores de una variable  (ASC o DEC))
        ...
        row += 1
        ws1.write_merge(row, row, 12, 16, "Compras Internas Afectadas sólo Alícuota General + Adicional",
                        sub_header_style)
        ...
        ws1.write(row, col + 17, invoice.float_format((
                base_general + base_genel_mas_adicional + base_reducido + tot_base_exentas + tot_base_no_gra + total_impo)),
                  result_right)
        ws1.write(row, col + 18, invoice.float_format(
            (tax_general + tax_genel_mas_adicional + tax_reducido + tot_iva_exentas + tot_iva_no_gra + total_iva_impo)),
                  result_right)
        ws1.write(row, col + 19, invoice.float_format((
                ret_general + ret_genel_mas_adicional + ret_reducido + tot_reten_exent + tot_reten_no_gra + tot_reten_impo)),
                  result_right)
