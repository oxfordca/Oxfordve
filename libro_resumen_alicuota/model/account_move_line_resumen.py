...
class AccountMove(models.Model):
    ...
    def suma_alicuota_iguales_iva(self):
        ...
        if self.move_type in ('in_invoice', 'in_refund', 'in_receipt', 'out_receipt', 'out_refund', 'out_invoice'):
                        ...
                        if self.state != "cancel":
                            ...
                            if tipo_alicuota == "exempt":
                                if det_fac.tax_ids.aliquot == "exempt":
                                    ...
                                    # raise UserError(_('alicuota : %s')%total_exento)
            ...
            # raise UserError(_('alicuota 2: %s')%total_exento)
            values = {
                ...
                'invoice_id': self.id,
                'vat_ret_id': self.retention_iva_id.id,
                'nro_comprobante': self.retention_iva_id.name,
                'porcentaje_ret': porcentaje_ret,
                'total_ret_iva': total_ret_iva,
                'type': self.move_type,
                'state': self.state,
                'state_voucher_iva': self.retention_iva_id.state,
                'tipo_doc': tipo_doc,
                ...
                'fecha_comprobante': self.retention_iva_id.iva_date,
