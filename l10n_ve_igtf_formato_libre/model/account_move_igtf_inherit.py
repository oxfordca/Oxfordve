...
class AccountMove(models.Model):
    ...
    formas_pago = fields.Many2many('account.payment.method',compute='_compute_metodos_pago')
    ...
    def _compute_metodos_pago(self):
        self.formas_pago=self.env['account.payment.method'].search([('payment_type','=','inbound'),('calculate_wh_itf','=',True)])

    def monto_base(self, metodo_id):
        total_monto_base = 0
        if self.igtf_ids:
            for det in self.igtf_ids:
                if det.metodo_pago.id == metodo_id:
                    total_monto_base += det.monto_base
        return total_monto_base

    def monto_ret(self, metodo_id):
        total_monto_ret = 0
        if self.igtf_ids:
            for det in self.igtf_ids:
                if det.metodo_pago.id == metodo_id:
                    total_monto_ret += det.monto_ret
        return total_monto_ret
        
    def bcv(self,metodo_id):
        resultado=0
        if self.igtf_ids:
            for det in self.igtf_ids:
                if det.metodo_pago.id==metodo_id:
                    resultado=det.move_id.os_currency_rate
        return resultado
    ...
    """def _compute_total_tax_org(self):
        ...
        for selff in self:
            for line in selff.line_ids:
                if line.account_id.id ==1957:
                    valor=valor+abs(line.balance)
            selff.total_impuesto_org=valor"""

    def _compute_sub_total_org(self):
        for selff in self:
            selff.sub_total_org=self.amount_total #selff.total_impuesto_org+selff.total_lineas_org
            ###selff.sub_total_org=selff.amount_total=selff.total_impuesto_org+selff.base_imponible_org+selff.exemto_org
...
class AccountMoveIgtf(models.Model):
    ...
    monto_base = fields.Float(digits=(12, 4))
    porcentaje = fields.Float(digits=(12, 4))
    monto_ret = fields.Float(digits=(12, 4))