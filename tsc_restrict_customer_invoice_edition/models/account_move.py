...
class AccountMove(models.Model):
    ...
    def write(self, vals):
        ...
        # 1. Excepciones de contexto: Permitir si la escritura es interna del motor de Odoo
        # (ej. Reversión/Nota de crédito, Publicación/Confirmación o Recálculo de líneas dinámicas)
        if (
            self._context.get('active_model') == 'account.move.reversal' or 
            self._context.get('building_reversal') or
            self._context.get('tracking_disable') or
            self._context.get('check_move_validity') is False
        ):
